...
fn read_inputs_from_args(args: Vec<String>, model: &mut Model) {
    ...
    } else if args.len() == 3 && args[1] == "--t3g" {
        match fs::read_to_string(&args[2]) {
            Result::Ok(s) => {
                model.t3grammar_state.definition_textarea.select_all();
                model.t3grammar_state.definition_textarea.cut();
                model.t3grammar_state.definition_textarea.insert_str(s);
            }
            Err(e) => {
                eprintln!("{e}");
                std::process::exit(1);
